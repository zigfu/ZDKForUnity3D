...
public class ZigScrollingMenu : MonoBehaviour
{
    public ZigFader fader;
    ...
    void Awake()
    {
        ...
        selector = gameObject.AddComponent<ZigItemSelector>();
        selector.fader = fader;
    
        // push detector, if we need one
        if (SelectOnPush || PushSlide) {
            if (null == GetComponent<ZigPushDetector>()) {
                gameObject.AddComponent<ZigPushDetector>();
            }
        }
		
		// fader for the pushslider, if we need one
		if (PushSlide) {
			pushslideFader = gameObject.AddComponent<ZigFader>() as ZigFader;
	...
	{
		if (PushSlide) {
			pushslideFader.MoveTo(gameObject.GetComponent<ZigPushDetector>().ClickPosition, 0.5f);
